...
_is_allowed = client_gem.HelucrypticApp._is_allowed


def test_allowed_when_mode_off(monkeypatch):
    monkeypatch.setattr(client_gem, "get_contact", lambda u: None)
    assert _is_allowed(_app(False), "bob") is True


def test_blocked_when_unverified(monkeypatch):
    monkeypatch.setattr(client_gem, "get_contact", lambda u: types.SimpleNamespace(verified=False))
    assert _is_allowed(_app(True), "bob") is False


def test_allowed_when_verified(monkeypatch):
    monkeypatch.setattr(client_gem, "get_contact", lambda u: types.SimpleNamespace(verified=True))
